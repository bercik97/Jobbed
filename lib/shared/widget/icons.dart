import 'package:flutter/cupertino.dart';
import 'package:flutter/material.dart';
import 'package:jobbed/shared/libraries/colors.dart';

// BLACK //
Icon iconBlack(IconData iconData) { return Icon(iconData, color: Colors.black); }
Icon icon30Black(IconData iconData) { return Icon(iconData, size: 30, color: Colors.black); }


// WHITE //
Icon iconWhite(IconData iconData) { return Icon(iconData, color: WHITE); }


// GREEN //
Icon iconGreen(IconData iconData) { return Icon(iconData, color: GREEN); }
Icon icon30Green(IconData iconData) { return Icon(iconData, size: 30, color: GREEN); }
Icon icon50Green(IconData iconData) { return Icon(iconData, size: 50, color: GREEN); }


// RED //
Icon iconRed(IconData iconData) { return Icon(iconData, color: Colors.red); }
Icon icon20Red(IconData iconData) { return Icon(iconData, size: 20, color: Colors.red); }
Icon icon30Red(IconData iconData) { return Icon(iconData, size: 30, color: Colors.red); }
Icon icon50Red(IconData iconData) { return Icon(iconData, size: 50, color: Colors.red); }


// ORANGE //
Icon iconOrange(IconData iconData) { return Icon(iconData, color: Colors.orange); }
Icon icon30Orange(IconData iconData) { return Icon(iconData, size: 30, color: Colors.orange); }
Icon icon50Orange(IconData iconData) { return Icon(iconData, size: 40, color: Colors.orange); }
