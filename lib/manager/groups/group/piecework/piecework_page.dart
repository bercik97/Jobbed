...
class _PieceworkPageState extends State<PieceworkPage> {
  ...
  void initState() {
    ...
    super.initState();
    _loading = true;
    _employeeService.findAllByGroupIdForPieceworkView(_model.groupId).then((res) {
      setState(() {
        _employees = res;
  ...
  Future<Null> _refresh() {
    return _employeeService.findAllByGroupIdForPieceworkView(_model.groupId).then((res) {
      setState(() {
        _employees = res;
        _employees.forEach((e) => _checked.add(false));
        _filteredEmployees = _employees;
